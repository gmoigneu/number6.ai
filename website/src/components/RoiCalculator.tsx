...
export default function RoiCalculator() {
	...
	const [values, setValues] = useState<number[]>(
		INPUTS_BASE.map((i) => i.defaultValue),
	);
	...
	const calculations = useMemo(() => {
		...
		const roi =
			annualCost > 0 ? ((totalAnnualValue - annualCost) / annualCost) * 100 : 0;
		const paybackPeriod =
			totalAnnualValue > 0 ? (annualCost / totalAnnualValue) * 365 : 0;
				...
				<div className="space-y-8">
					...
					<div className="bg-primary/5 rounded-2xl p-6 lg:p-8 space-y-6">
						<div className="flex items-baseline gap-2">
							<span className="font-heading text-5xl lg:text-6xl font-bold tracking-[-2px] text-[#C45A3B] tabular-nums">
								{formatCurrency(
									Math.round(calculations.totalAnnualValue / 1000) * 1000,
									currency,
								).replace(/\d(?=(\d{3})+$)/g, "$&,")}
								k
							</span>
						...
						<div className="flex flex-wrap gap-4 text-sm">
							<div className="flex items-center gap-2 bg-background px-3 py-1.5 rounded-full">
								...
								>
									<path
										...
										strokeWidth={2}
										d="M16 7h6v6m-8-6h6v6m-8-4h6v6m-8-2h6v6"
									/>
								</svg>
							...
							<div className="flex items-center gap-2 bg-background px-3 py-1.5 rounded-full">
								...
								<span className="font-body">
									<strong className="font-semibold">
										{Math.round(calculations.paybackPeriod)} days
									</strong>{" "}
									payback
								</span>
							</div>
					...
					<div className="space-y-4">
						...
						<div className="space-y-3">
							{results.map((result) => {
								const percentage =
									calculations.totalAnnualValue > 0
					...
					<div className="pt-4">
						...
						<p className="mt-3 font-body text-xs text-muted-foreground">
							Based on your inputs, we&apos;d be happy to refine these estimates
							with more details about your specific workflows.
						</p>
					</div>
				</div>
			</div>
